/**
 * Mobile filter bottom sheet for the asset archive page.
 */
(function () {
  ...
  function getForm() {
    return document.getElementById('archiveFilterForm');
  }
  ...
  function categoryHasValue(cat, draftVal) {
    var def = FILTER_DEFAULTS[cat.id];
    if (def !== undefined) {
      return String(draftVal) !== String(def);
    }
  ...
  function onClear(e) {
    e.preventDefault();
    if (!activeCtx) return;
    activeCtx.draft = activeCtx.cats.map(function (cat) {
      return FILTER_DEFAULTS[cat.id] || '';
    });
    commitFilters(true);
  }

  function setupDock() {
    document.body.classList.add('archive-page-open', 'has-archive-mobile-dock');

    var dock = document.querySelector('.archive-mobile-dock');
    ...
    document.querySelectorAll('[data-archive-dock-filter]').forEach(function (btn) {
  ...
  window.ATSArchiveMobileFilters = {
    open: openSheet,
    close: closeSheet,
